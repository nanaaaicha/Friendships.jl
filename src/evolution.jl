        ...
        for j in i+1:length(people)
            ...
            if rand() < prob
               people[i].friends = union(people[i].friends, [j])
                people[j].friends = union(people[j].friends, [i]) # push! m'affiche des doublons
...
function similarity(p1::Person, p2::Person)
    ...
    #j'ai tout changé parce que jutilise NamedTuple et jai rechangé parce que c'est pas très pratique, j'aurais aimé mieux le vendre ;)
end
